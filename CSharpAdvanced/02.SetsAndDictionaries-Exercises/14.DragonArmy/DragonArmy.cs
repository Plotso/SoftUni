namespace _14.DragonArmy
{
    ...
    public class DragonArmy
    {
        ...
        public static void Main()
        {
            ...
            var numbersOfDragons = int.Parse(Console.ReadLine());
            ...
            for (int i = 0; i < numbersOfDragons; i++)
            {
                ...
                var dragonDamage = dragon[2]
                    .Equals("null") ? DefaultDamage : int.Parse(dragon[2]);
                var dragonHealth = dragon[3]
                    .Equals("null") ? DefaultHealth : int.Parse(dragon[3]);
                var dragonArmor = dragon[4]
                    .Equals("null") ? DefaultArmor : int.Parse(dragon[4]);

                if (allDragons.ContainsKey(dragonType))
                {
                     allDragons[dragonType][dragonName] =
                     new int[] { dragonDamage, dragonHealth, dragonArmor };
                    
                }
                else
                {
                    allDragons[dragonType] =
                        new SortedDictionary<string, int[]>() { { dragonName, new int[]
                        { dragonDamage, dragonHealth, dragonArmor } } };
                }
            }
        ...
        private static void PrintAllDragons(Dictionary<string, SortedDictionary<string, int[]>> allDragons)
        {
            foreach (var dragonType in allDragons)
            {
                ...
                {
                    dragonTypeInfo.Append($"-{dragon.Key} -> damage: {dragon.Value[0]}" +
                ...
                Console.WriteLine($"{dragonType.Key}::" +
                    $"({avrDamage:f2}/{avrHealth:f2}/{avrArmor:f2})");
